...
@component('layouts.component.form')
    @slot('isfile', false)
    @slot('action', !isset($data['detail']) ? route('spp.store') : route('spp.update'))
    @isset ($data['detail'])
        @slot('method','PUT')
    @else
        @slot('method','POST')
                                    ...
                                    $(document).find('#list-ganjil-'+key.id).append(
                                        `<li class="list-group-item">${index.bulan}
                                            <div class="icheck-primary d-inline float-right">
                                                <input class="spp_payment" type="checkbox" data-spp_cost="${spp_cost}" value="${index.id}" name="payment[]" id="${index.id}">
